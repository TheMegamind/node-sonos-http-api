...
function HttpAPI(discovery, settings) {
  ...
  this.requestHandler = function (req, res) {
    ...
    .then((response) => {
      if (!response || response instanceof http.IncomingMessage) {
        response = { status: 'success' };
      } else if (Array.isArray(response) && response.length > 0 && response[0] instanceof http.IncomingMessage) {
        response = { status: 'success' };
      }
  ...
  function invokeWebhook(type, data) {
    const typeName = settings.webhookType || 'type';
    const dataName = settings.webhookData || 'data';
    ...
    try {
      ...
      // Player.toJSON() can reference a transiently undefined coordinator
      // during topology changes — swallow and skip this notification.
      logger.debug(`invokeWebhook: skipping ${type} event, JSON serialization failed: ${err.message}`);
      return;
    }
