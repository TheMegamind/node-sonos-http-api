...
function announcePreset(system, uri, preset, duration) {
  if (!preset.players || preset.players.length === 0) {
    return Promise.reject(new Error('announcePreset: preset must have at least one player'));
  }
  ...
  const oneGroupPromise = new Promise((resolve, reject) => {
    ...
    abortTopologyWait = () => {
      clearTimeout(timer);
      system.removeListener('topology-change', onTopologyChanged);
    };
  });
  ...
  return coordinator.pause()
    ...
    .then(() => {
      if (hasReachedCorrectTopology(system.zones)) {
        abortTopologyWait();
        return;
      }
    ...
    .then(() => {
      return backupPresets.reduce((promise, preset) => {
        ...
        return promise
          .then(() => system.applyPreset(preset))
