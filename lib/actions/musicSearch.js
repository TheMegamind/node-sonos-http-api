...
function getAccountId(player, service)
{
  ...
  if (service != 'library') {
    return got(player.baseUrl + '/status/accounts')
      .then((res) => {
        var actLoc = res.body.indexOf(player.system.getServiceType(serviceNames[service]));
       
        if (actLoc != -1) {
          var idLoc = res.body.indexOf('<UN>', actLoc)+4;
          var snLoc = res.body.indexOf('SerialNum="', actLoc)+11;
