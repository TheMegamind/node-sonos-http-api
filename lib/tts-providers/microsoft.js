...
function microsoft(phrase, voiceName) {
  ...
  // Microsoft TTS outputs .wav — override the .mp3 extension from tts-cache
  const { filepath: fpMp3, uri: uriMp3 } = getTTSCache('microsoft', phrase, settings.name);
  const filepath = fpMp3.replace(/\.mp3$/, '.wav');
  const expectedUri = uriMp3.replace(/\.mp3$/, '.wav');

  const cached = checkTTSCache(filepath, expectedUri);
  ...
  return promise.then(() => {
    ...
    return request({
      ...
      .then(() => resolveTTSFile(filepath, expectedUri))
      .catch((err) => {
        logger.error(err);
        throw err;
      });
